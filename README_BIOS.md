...
If you want only the command lines and no description, head over to the `commands_BIOS.txt` file (on this repo).
...
Type `t[ENTER]` to choose the partition type, `4[ENTER]` to choose a BIOS bootI partition. The first partition is done, it is the partition that will allow the system to boot. We will now create the SWAP partition, a partition allowing your system to use some of your hard drive as your RAM. You can skip this step, but as it might create problems I encourage you to just set the partition size to 1 Gb instead of just not creating it.
